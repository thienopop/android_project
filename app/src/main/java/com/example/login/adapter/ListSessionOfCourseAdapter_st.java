...
public class ListSessionOfCourseAdapter_st extends ArrayAdapter<SessionInfo> {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (session != null) {
            ...
            holder.txtStartDate.setText("Ngày học: "+session.getSessionDate());

            // Chuyển Duration (Integer/int) sang String
            // Có thể thêm đơn vị nếu cần: String.valueOf(session.getDuration()) + " phút"
            holder.txtDuration.setText("Thời lượng(phút ): "+String.valueOf(session.getDuration()));

            // Gán Trạng thái (String)
            holder.txtStatus.setText("Status: "+session.getStatus());
        }
